...
## [2026-09-14 / шаг 27] Карточка доски — конечный автомат; архив обратим; удаление реально
**Запрос:** «при запущенном анализе всё равно показывается „Запустить анализ“ — должна быть плашка
и остановка», «в архиве должны быть „вернуть в воронку“ и „удалить идею“», «выход из карточки
уводит на экран записи идеи, а не в список».
**Результат:**
- `card-actions.ts` — чистый планировщик `planCardActions(stage, job, jobKnown)`: queued/running →
  плашка + «Остановить» + «В архив»; paused → «Продолжить»; failed → «Запустить» (идемпотентность
  `/run` не покрывает завершённые); archived → **«Вернуть в воронку» + «Удалить идею»**; при
  неизвестном статусе запуска нет вовсе (иначе карточка мигала «Запустить» → «Остановить» ~200мс)
- `useIdeaActions` + `useActionFlash` — действия и однострочный ответ доски; архив в два касания
  (4с на подтверждение, сброс по таймеру) с «Вернуть» в тосте (8с); ошибки — тостом над нижним
  баром, а не `<p>` над списком, откуда уезжали за вьюпорт
- `server/api/ideas/[id]/index.delete.ts` — 204, отказ при живой задаче (`IDEA_JOB_ACTIVE`) и для
  неархивированной (`IDEA_NOT_ARCHIVED`); каскады контракта уносят историю/прогоны/отчёты/аудио
- **найденное при проверке удаления:** архив оставлял задачу в `queued`, и удаление становилось
  невозможным навсегда. PATCH при `funnel_stage=archived` теперь отменяет активные задачи
  (`cancelJob`, остановка на границе шага) — иначе прогон писал бы выводы в убранную идею
- `IdeaCard.vue` — крошка «← Список идей» вела на `/` (это и был жалобный бах), теперь `/ideas`;
  чтобы файл влез в `max-lines`, из него вынесены `IdeaMvpPanel.vue` (MVP TZ §10) и
  `IdeaOutputsPanel.vue` (журнал агентов) — 479 → 337 строк, обе панели грузят своё сами
- `MobileBottomBar.vue` — пилюля `inset-1` вместо `inset-0`: при 54px внутри 56px бара она
  заполняла его целиком и выглядела «вылезающей» за край
- `FunnelBoard.vue` — чип «Архив» грузится на монтировании (иначе доска, где всё убрано, не даёт
  в архив попасть) и не дублируется (`archived` исключён из полоса стадий); счётчик «N из 10»
  влит в `h1`, пустые стадии скрыты, полоса прокручивается; `activeCount` больше не считает
  то же самое вторично
- `run.post.ts` — текст `fix` больше не обещает несуществующий контроль: отсылает к «Вернуть в воронку»
**Проверка (живьём, не «должно работать»):** создана тестовая идея; idle → «Запустить анализ»+«В архив»;
после `/run` → плашка «Идея в очереди», `aria-busy=true`, кнопки запуска **нет**; клик «В архив» →
«Подтвердить архив» + тост подсказки, повторный клик → карточка исчезла, тост «Идея в архиве · Вернуть»;
вкладка «Архив (2)» → карточки с «Вернуть в воронку»/«Удалить идею»; диалог удаления с описанием
последствий; `DELETE` → 204. Замер пилюли: `top 777 / h 54` внутри бара `top 776 / h 56`.
Кнопки держат 44px и одну строку на 390/360/320 (было: 2 строки на 390).
Гейт: lint 37 → **36** (сплит IdeaCard снял его max-lines), typecheck exit 0, vitest
**6 failed / 74 passed** (те же 6 падений baseline, +14 новых зелёных: 10 планировщик + 4 сортировка + …),
e2e 4/4. Тестовые строки удалены: активных 0, в архиве 0.


## [2026-09-14 / шаг 26] P0 доски: карточка вертикальным стеком + семантическая сортировка
**Запрос:** «давай по порядку» — пункт (a) из критики: читаемость карточки на мобильном + порядок.
**Результат:**
- `FunnelBoard.vue`: ряд карточки заменён вертикальным стеком (заголовок / проблема / чипы,
  затем ряд действий через `border-t`, кнопки равные `flex-1`, `h-11` вместо `h-10` — заодно
  закрыт 40px тач-таргет). Попутно снят `hover:shadow-sm` (DESIGN запрещает тени на статике).
  Причина коллапса: `flex-wrap` не срабатывал — у `flex-1` базис 0% против `shrink-0` кнопок
- `shared/priority.ts` (новый, без зависимостей — vitest не резолвит `~~`): `compareIdeasByPriority`
  high → medium → low, внутри — новые сверху, тай-брейк по id; `index.get.ts` сортирует им
  вместо `orderBy(priority)` (текстовая колонка давала high, low, medium — проверено в Postgres)
- `server/utils/idea-sort.test.ts`: 4 теста (порядок, новые сверху, неизвестный в конец, стабильность)
**Проверка:** юнит 4/4; замер в живой вёрстке инъекцией разметки (только клиент, без записи):
колонка заголовка 248px из 312px карточки = 79% (было 10px / 3%); скрин подтверждает.
Гейт: lint 37 = baseline (моих файлов нет), typecheck exit 0, vitest 6 failed / 64 passed
(те же 5 файлов, +4 новых зелёных), e2e 4/4. Порядок живьём не показать — dev-БД пуста.
**Не взято (следующие пункты):** (b) двухшаговый архив + undo + «вернуть в воронку»; (c) фильтр
9 чипов → radiogroup/полоса + слияние счётчиков; шрифт с кириллицей; живой статус (SSE/полл).


## [2026-09-14 / шаг 25] Нижняя навигация — компактная пилюля, переходы экранов, правки экрана идеи
**Запрос:** рецензия экрана «Новая идея» (CTA, микрофон, интро, счётчик, auto-grow, сегмент,
дубль иконок) + закруглённая компактная панель с выделением текущего экрана и анимациями +
вопрос «как реализовать».
**План:** nav + `layoutId`-пилюля вместо `UiTabs` (табы без tabpanel ломают семантику и
`aria-current`); настройки отдельным экраном оставлены по решению человека (TZ §7 п.7).
**Результат:**
- `MobileBottomBar.vue`: плавающая пилюля `h-14 max-w-420px rounded-full`, активный пункт —
  `motion.span layout-id="mobile-nav-pill"` (spring) + `font-medium` + `aria-current="page"`;
  «Новая идея» — `lucide:plus` (микрофон остался только в поле ввода); иконки 16px, метки с
  `truncate` как страховка
- `default.vue`: переход `AnimatePresence mode="wait"`, `y:8/opacity` 180мс; при `?motion=off`
  или `prefers-reduced-motion` обёртки нет вовсе (иначе opacity флапает capture-скрины);
  иконка шапки тоже `plus`
- `CreateIdeaForm.vue`: интро в одну строку + `UiTooltip` с полным текстом; счётчик только при
  вводе; textarea `field-sizing:content rows=3 max-h-64`; плейсхолдер с примером; микрофон
  `bg-surface text-foreground`; внутренняя submit-иконка удалена (primary на экране одна);
  сегмент — `ring-1 ring-primary` + `font-bold` + галочка + `layout-id="priority-pill"`; добавлен
  видимый `<label for="transcript">`
- `e2e/smoke.spec.ts`: тест «ровно один `aria-current` в мобильном баре» (мобильный вьюпорт —
  `md:hidden` убирает бар из дерева доступности на десктопе)
**Проверка (§15):** capture desktop 1440 + mobile 360/320, `impeccable detect --json` → `[]`,
клавиатурный проход доходит до навигации, `impeccable-finish-reviewer` → `disposition: fix`,
7 пунктов применены одним пакетом (пилюля/inset, иконка-`?` 36px, label поля, tooltip-blue,
тень по формуле, короткий плейсхолдер). Гейт: lint по файлам чисто, typecheck exit 0,
vitest 6/60 = baseline, e2e 4/4.
**Циклы отладки:** (1) новый e2e-тест падал с «0 элементов» — бар `display:none` на десктопе
отсутствует в дереве доступности, лечится `setViewportSize` + `toBeVisible`; (2) на скрине
мерещилось «Список ид…» — zoom показал, что это бейдж Nuxt DevTools перекрывал подпись;
DOM-замер (`scrollWidth == clientWidth`) + удаление devtools-острова перед кадром.
**Открыто, нужно решение человека:** шрифт. Outfit без кириллического сабсета — весь экран
рендерится системным фолбэком (подтверждено ревьюером по скриншотам, DEVLOG шаг 11 держал
вопрос открытым): либо заменить display/body-фейс с кириллицей в DESIGN.md → `@theme`,
либо зафиксировать фолбэк как принятый вид.
...
## [2026-09-14 / шаг 28] Обратная связь проверяющего: фазы по-человечески, материалы, читаемая эффективность, мобильная вёрстка

**Пост-мердж фикс (стенд):** после пуша прогон, заведённый на публичном стенде, упал на первом же шаге —
`relation "public.checkpoints" does not exist`. Причина: таблицы LangGraph-чекпоинтов создаёт
`ensureCheckpointerTables()`, который вызывал только `worker-cli.ts`; Nitro-плагин постоянного воркера
(он и есть сервис `worker` в compose) — нет. Значит на стенде не мог завершиться НИ ОДИН прогон, а локально
дефект не проявлялся, потому что таблицы там создавались CLI-путём. Проверено удалением: в dev-БД таблицы
удалены (4 → 0), dev поднят с `WORKER_MODE=true` (тот же путь, что и прод) — таблицы восстановлены самим
плагином (0 → 4), лог «обработчик очереди запущен», `/api/health` 200. Ветка `fix/checkpoints-on-worker-boot`.

**Ветка:** `feat/report-e2e` (4 коммита: `972840f` fix(pipeline) → `ff108ac` feat(ui) → `4028fd0` fix(ui) → `a32ebb4` fix(report)); смёржена в `main` через `--ff-only`, `ed2ccec..a32ebb4`, ветка удалена после пуша.

**Запрос:** пять пунктов от проверяющего — (1) человеческие названия фаз, (2) MD-материалы каждой фазы в карточке (главный провал прошлой версии: «не увидел результатов работы агентов»), (3) человекочитаемая карточка эффективности, (4) скрыть технические окна, (5) 390px без горизонтального скролла. Сквозное требование владельца: идея **стабильно доводится до отчёта**.

**План:** сначала пункт 0 — сделать прогон воспроизводимым и проверяемым (иначе пункты 1–3 нечем проверить), затем 1→5 строго по порядку, после каждого — lint/typecheck/vitest.

**Результат (что было найдено — всё измерено, не угадано):**

1. **Отчёт не собирался по пяти независимым причинам**, ни одна не была «UI-шником»:
   - LLM-провайдер зашит в код (`routerai.ru` + `z-ai/glm-5.3-flash`); ключ читал каталог моделей (`GET /models` → 200), но генерацию шлюз отвергал (`POST /chat/completions` → 401 на любой модели, при Bearer и при `x-api-key`, на 5 вариантах пути). Первый же шаг умирал. Новый провайдер вынесен в `config/llm.ts` (TZ §6: модели — в /config), проверен живым `callLlm` → валидный JSON за 3.4 c.
   - Выходы фаз писались в `agent_outputs` **только в конце успешного прогона**. Упавший на середине прогон не оставлял ничего — это и есть «проверяющий не увидел работы агентов». Запись перенесена в `makeNode` (фаза видна, как только отработала); тесты в `worker.test.ts` ждут строку ДО конца прогона.
   - Путь `retry-step` (replay) не вызывал сборку отчёта вовсе → «продолжить прогон» не давало отчёта.
   - `executionStatus` не сбрасывался при успехе (идея навсегда «идёт анализ»), и `job.error` не очищался → у `done`-задачи висел красный алерт «HTTP 401» от прошлой попытки.
   - `GET /api/ideas/:id/calculations` падал 500 «Do not know how to serialize a BigInt» (колонка `seed`): экран эффективности был мёртв с момента создания таблицы — его просто никогда не дёргали на непустой БД.
2. **Расчёт эффективности врали в рекомендациях.** `bootstrapMeanDiff(baseTimes, [vm], seed)` — группа сравнения из одного элемента, guard «n < 2» возвращал CI `[0, 0]` **всегда**; правило «CI включает без изменений» срабатывало всегда → любая идея приземлялась в `validate_first`. Замер до/после на одном входе: CI `[0;0]` → `[3,53; 3,92]` (10 000 ресемплирований), вердикт `validate_first` → `develop`. CI сценариев раньше подставлялся точечной оценкой (`{lower: effect, upper: effect}`) — теперь считается тем же bootstrap'ом.
3. **Секции отчёта никогда не рендерились.** Nuxt автоимпортирует только `app/components/**`, а `StopFactorsSection`, `WeaknessesSection`, `NextStepsSection`, `IdeaMvpPanel`, `IdeaOutputsPanel` живут в `app/features/**` и не импортировались — Vue молча пропускал неизвестные теги. Отсюда «в отчёте ничего нет». Найден сканом всех `*.vue` (использованные, но не импортированные имена); теперь скан пуст.
4. Пункты 1–4 сделаны: `shared/phase-names.ts` (единственный источник, тест сверяет его с `config/pipeline`), `GET /api/ideas/:id/phases/:role/materials` + `MaterialViewer` (блоки вместо HTML — `v-html` по текстам модели это XSS-поверхность; jsonb не хранит порядок ключей → обход канонический), `shared/field-labels.ts` (в материалах больше нет `willingnessToPay: medium`), серверная view-модель `EfficiencyCard` (клиент ничего не пересчитывает, TZ §5), `useDevMode` (по умолчанию ВЫКЛ) + «Технические детали» в отчёте в один клик, сводка отчёта в карточке идеи.
5. Честные отступления: URL материалов — `/api/ideas/:id/phases/:role/materials`, а не `/api/jobs/[id]/steps/[stepId]/...`, потому что таблицы «шаг с полем output» в схеме нет (выход роли живёт в `agent_outputs` по `ideaId + role`); `llm-executor.test.ts` — стаб-контекст дополнен (`db`-двойник), ассерты не изменены; переключатель «Режим разработчика» остался штатным примитивом 24px (по ТЗ требование ≥44px к **кнопкам действий**; попытку расширить зону через `after:-inset` проверил поведением и отменил — клик мимо box не срабатывает, ложной «соответствует» не оставляю); STT (`server/utils/stt.ts`) остался на старом провайдере и сегодня не проверен.

**Проверка:**
- `pnpm exec vitest run` — **127/127, 16 файлов** (baseline сессии: 6 падений + 3 файла не собирались из-за отсутствующих алиасов `~~`/`~` в vitest и отсутствия тестовой БД 5434; поднял контейнер, накатил 37 миграций).
- `pnpm typecheck` — exit 0; `eslint` по изменённым файлам — 0 ошибок (45 на всё дерево = baseline 36 в чужих файлах + 9 мои, разобраны).
- Реальный прогон end-to-end на новом ключе: `done`, 7 фаз в `agent_outputs` с **разными** `created_at` (05:07:27 → 05:12:52 — доказательство записи по мере выполнения), `run_calls.component = llm:<шаг>`, идея `decision|paused`, отчёт v3, `calculations[0].view`: CI usable, «6 мин → 2,4 мин (60% быстрее)», «95%: 3,4 мин – 3,8 мин», вердикт расчёта «Развивать».
- Браузер (agent-browser, реальные клики): карточка — 7 кнопок «Материалы», сводка отчёта есть, тех-id на экране нет (`orchestrator_plan`/`agent_outputs` в innerText — 0), ложного алерта «Прогон остановился» нет; диалог материалов открывается и заполнен; отчёт — стоп-факторы и слабые места рендерятся.
- 390px: `scrollWidth == clientWidth == 390` на «Новая идея», «Список идей», «Настройки», карточка, отчёт; вылезающих элементов 0; контролов <44px после правок 0 (было 8: голосовой ввод, чипы приоритета, чипы фильтра, кнопка «Подробнее»); инпуты ≥16px.
- Публичный стенд не тронут: коммиты на ветке, push — за владельцем (push = деплой, CI нет, Dokploy тянет `main`).

- Проза критика в отчёте несла сырые токены расчёта («База (meanMinutes ~6 мин, rowCount 200)», «3.57 мин экономии») — это пункт 4, но лечится не промптом: `humanize-report.ts` на границе `report.get.ts` меняет написания, смысл не трогает. Первый вариант регулярки портил версии (2.5.1 → 2,5,1) — поймано собственным тестом, паттерн переписан с границами «не цифра и не точка»; живая проверка ответа: 0 сырых токенов, 0 чисел с точкой при «мин».
**Fixes:** алиасы `~~`/`~` в `vitest.config.ts` + `paths` в корневом `tsconfig.json` (чинит и `pnpm worker` под tsx); ключ LLM в `test.env` читается из `.env` без мутации `process.env` (иначе dev-база 5433 утёкла бы в тесты вместо 5434); `seed` → строка на границе API; `error: null` при завершении; тип `EfficiencyView` вынесен в `shared/` (правило «server не импортируется на клиент»); `useState` принимает только строковый ключ — страница `ideas/[id]` получила `definePageMeta({ key })`, иначе кэш показывал данные предыдущей идеи; на вложенных маршрутах карточка больше не дублируется целиком. Отдельный инцидент: машина владельцем была забита до 8,1/9,2 ГБ свопа — одновременно жили 2 dev-сервера (один раздут до ~3,2 ГБ RSS) и лишние воркеры; после уборки (по одному) своп упал до 5,7 ГБ. Один замер был загрязнён этим (задачи разбирал воркер со старым кодом) — перемерил начисто, вывод про «вырожденный CI» подтвердился на свежих данных.
