...
**Проверка:** этот коммит docs-only (правило п.6), запушен в `main` **без** `compose.deploy`.
Критерий: в `deployment.allByCompose` появляется пятая запись позже времени пуша. Если нет —
автодеплой не доказан; дальше либо Deploy Webhook URL из панели, либо осознанная кнопка Deploy.
**Fixes:** (1) гипотеза «ключ не принимается» опровергнана: 401 был следом запроса на `/api/composed`;
(2) вывод «деплойка с заголовком коммита = сработал webhook» признан ложным.
